...
try:
	...
	payload = {"name": svcName, "sourceDataset": srcDataset, "type": "scheduled", "recursive": False, "schedules": [{"keepDestination": keepSS, "keepSource": keepSS, "cron": cron, "replicationDisabled": False, "scheduleName": schName, "disabled": False, "force": False}], "force": False, "scheduleName": svcName, "takeoverOrphanedSnapshots": False}
	...
	if checkTask(vcip=vcIP, tid=taskID) == 1:
		update_testrail(run_id=TRrun_id, cid="312540", result=5, msg=jenkinsLastBuildNumber(tn = testCaseName, jj = jenkinsJob, sn = suiteName))
	update_testrail(run_id=TRrun_id, cid="312540", result=1, msg=jenkinsLastBuildNumber(tn = testCaseName, jj = jenkinsJob, sn = suiteName))
	...
	if deleteService(vcip=vcIP, nsid=srcHostID, sn=svcName, body=payload) == 1:
		update_testrail(run_id=TRrun_id, cid="312539", result=5, msg=jenkinsLastBuildNumber(tn = testCaseName, jj = jenkinsJob, sn = suiteName))
	update_testrail(run_id=TRrun_id, cid="312539", result=1, msg=jenkinsLastBuildNumber(tn = testCaseName, jj = jenkinsJob, sn = suiteName))
	time.sleep(5)
