...
try:
	...
	# Creating remote replication service on source host
	print "Create Service ..."
	payload = {"name": svcName, "sourceDataset": srcDataset, "destinationDataset": destDataset, "type": "scheduled", "remoteNode": {"host": destHost, "port": 8443, "proto": protocol}, "recursive": False, "schedules": [{"keepDestination": keepSS, "keepSource": keepSS, "cron": cron, "replicationDisabled": False, "scheduleName": schName, "disabled": False, "force": False}], "force": False, "scheduleName": svcName, "takeoverOrphanedSnapshots": False}
	...
	if checkTask(vcip=vcIP, tid=taskID) == 1:
		update_testrail(run_id=TRrun_id, cid="312606", result=5, msg=jenkinsLastBuildNumber(tn = testCaseName, jj = jenkinsJob, sn = suiteName))
	update_testrail(run_id=TRrun_id, cid="312606", result=1, msg=jenkinsLastBuildNumber(tn = testCaseName, jj = jenkinsJob, sn = suiteName))
	...
	if deleteService(vcip=vcIP, nsid=srcHostID, sn=svcName, body=payload) == 1:
		update_testrail(run_id=TRrun_id, cid="312601", result=5, msg=jenkinsLastBuildNumber(tn = testCaseName, jj = jenkinsJob, sn = suiteName))
	update_testrail(run_id=TRrun_id, cid="312601", result=1, msg=jenkinsLastBuildNumber(tn = testCaseName, jj = jenkinsJob, sn = suiteName))
	time.sleep(5)

	# Destroying filesystem on a destination host
	print "Destroy destination fs ..."
	destroyFS(vcip=vcIP, nsid=destHostID, pool=destPool, fs=destFs)
	time.sleep(5)
