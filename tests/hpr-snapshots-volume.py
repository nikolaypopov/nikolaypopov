...
print "*******************************************"
print "Test description: Create and destroy Scheduled Snapshots Service on top of iSCSI volume"
print "Sending API request to VC " + vcIP
print "*******************************************"

myFileName = os.path.realpath(__file__) # Get this filename and path

findTestCase = findLine(tl = "{}".format(myFileName), suite = "/koka/suites/suite-test.py") # Find test case related to this file name and path
...
try:
	...
	payload = {"name": svcName, "sourceDataset": srcVolume, "type": "scheduled", "recursive": False, "schedules": [{"keepDestination": keepSS, "keepSource": keepSS, "cron": cron, "replicationDisabled": False, "scheduleName": schName, "disabled": False, "force": False}], "force": False, "scheduleName": svcName, "takeoverOrphanedSnapshots": False}
	...
	if checkTask(vcip=vcIP, tid=taskID) == 1:
		update_testrail(run_id=TRrun_id, cid="312542", result=5, msg=jenkinsLastBuildNumber(tn = testCaseName, jj = jenkinsJob, sn = suiteName))
		resultPrint(1)
		exit(1)
	update_testrail(run_id=TRrun_id, cid="312542", result=1, msg=jenkinsLastBuildNumber(tn = testCaseName, jj = jenkinsJob, sn = suiteName))
	resultPrint(0)
