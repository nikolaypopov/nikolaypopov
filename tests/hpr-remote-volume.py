...
print "*******************************************"
print "Test description: Create and delete Remote Replication Service on top of iSCSI volume"
...
try:
	...
	if checkTask(vcip=vcIP, tid=taskID) == 1:
		update_testrail(run_id=TRrun_id, cid="312608", result=5, msg=jenkinsLastBuildNumber(tn = testCaseName, jj = jenkinsJob, sn = suiteName))
		resultPrint(1)
		exit(1)
	update_testrail(run_id=TRrun_id, cid="312608", result=1, msg=jenkinsLastBuildNumber(tn = testCaseName, jj = jenkinsJob, sn = suiteName))
	resultPrint(0)
	...
	destroyVolume(vcip=vcIP, nsid=destHostID, pool=destPool, vol=destVolName) == 1
